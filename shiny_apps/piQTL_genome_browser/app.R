...
Sys.setenv(TRACKS_DIR = file.path(getwd(), "tracks"))
dir.create(Sys.getenv("TRACKS_DIR"), showWarnings = FALSE, recursive = TRUE)
...
server <- function(input, output, session) {
  ...
  output$igvShiny_0 <- renderIgvShiny({
    ...
    f_LDs <-"./data/genome_annotations/LD_blocks_050.csv"
  ...
  observeEvent(input$addGwasTrackButton_DrugMTX, {
    printf(glue("---- Adding GWASTrack for {input$ppi} with {input$drug}"))
    # colors <- as.list(rep(c("black", "lightgray"), 17))
    f <- glue("https://raw.githubusercontent.com/ladyson1806/public_hosting/main/piQTL_mapping/formatted_for_genome_browser/{input$ppi}_{input$drug}_SNP_annotations.csv")
    tbl.gwas <- read.csv(f, sep=',')
    tbl.gwasTrack <- GWASTrack(glue('{input$ppi} with {input$drug}'), tbl.gwas, chrom.col=2, pos.col=3, pval.col=5, trackHeight=100)
    display(tbl.gwasTrack, session, id="igvShiny_0", deleteTracksOfSameName = TRUE)
  })
  
  observeEvent(input$addGwasTrackButton_noDrugMTX, {
    printf(glue("---- Adding GWASTrack for {input$ppi} without {input$drug}"))
    # colors <- as.list(rep(c("black", "lightgray"), 17))
    f <- glue("https://raw.githubusercontent.com/ladyson1806/public_hosting/main/piQTL_mapping/formatted_for_genome_browser/{input$ppi}_noDrug_SNP_annotations.csv")
    tbl.gwas <- read.csv(f, sep=',')
    tbl.gwasTrack <- GWASTrack(glue('{input$ppi} without {input$drug}'), tbl.gwas, chrom.col=2, pos.col=3, pval.col=5, trackHeight=100)
    display(tbl.gwasTrack, session, id="igvShiny_0", deleteTracksOfSameName = TRUE)
  })
