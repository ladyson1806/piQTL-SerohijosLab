"""
01d_barcode_mapping.py
----------------------------
Maps extracted barcodes to strain identifiers using Hamming distance matching
against a reference barcode library. Processes all experimental conditions in
parallel (Step 5 of the piQTL pipeline), producing per-condition read count
matrices and mapping quality logs.

Inputs:
    ../../data/pipeline/barcode_collection_with_ubarcodes.csv
        Reference barcode library with columns:
            - 'strain_number': unique strain identifier
            - 'revComp':       reverse complement barcode sequence

    /home/savvy/PROJECTS/PHD/DATA/FINAL_DATASET/barcode_extracted/{condition}/pooled/
        Per-condition folder of Bartender-extracted barcode files
        (*_barcode.txt), each with columns: barcode, count.

Outputs:
    ../../results/01_barcode_count/per_condition/{condition}_read_number.csv
        Wide-format read count matrix: strains (rows) × timepoints/samples (cols).
    ../../results/01_barcode_count/log/{condition}.log
        Per-sample mapping statistics:
        Condition, total_reads, mapped_reads, %_barcode_mapped, nb_strains.

Usage:
    python 01d_barcode_mapping.py

Dependencies:
    multiprocessing, os, signal, textdistance, numpy, pandas,
