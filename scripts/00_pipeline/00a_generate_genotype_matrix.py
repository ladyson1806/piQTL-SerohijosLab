"""
00a_generate_genotype_matrix.py
-----------------------
Processes and formats yeast genotype data from the She & Jarosz 2018 dataset for
use in piQTL mapping pipeline. Handles SNP positions, gene annotations (ORFs),
and genotypic matrices for a barcoded strain collection.

Inputs:
    ../../data/genotype_information/she_jarosz_2018/phasedGenotype_from_mat.csv
        Phased genotype matrix (strains × SNPs), no header.
    ../../data/genotype_information/she_jarosz_2018/variantPos_from_vcf.csv
        Tab-separated SNP position table with columns: chrom, position, REF, ALT.
    ../../data/genome_annotations/sgd_database/orf_coding_R64-3-1.csv
        SGD genome annotation table, must include columns:
        'genome_annotations', 'locus_id', 'name', 'sgd_id', 'chrom', 'start', 'end'.
    ../../data/pipeline/barcode_collection.csv
        Barcode-to-strain mapping table, must include a 'strain_number' column.

Outputs:
    ../../data/genotype_information/yeast_ORFs_dec2022.txt
        Filtered ORF annotations (CSV).
    (Commented out) yeast_snps_loc_dec2022.txt, piQTL_genotype_matrix_dec2022.txt

Usage:
    python 00a_generate_genotype_matrix.py

Dependencies:
    re, numpy, pandas, scipy, roman, IPython
"""
import re, roman 
import numpy as np
import pandas as pd
...
def format_gene_loc(gene_loc):
    ...
    gene_loc[['locus_id', 'name', 'sgd_id', 'chrom', 'start', 'end']].to_csv('../../data/genotype_information/yeast_ORFs_dec2022.txt', index=False)


def format_snps_loc(variant_pos):
    ...
    print('Table of genetic variants, positions and alleles:')
    print(variant_pos[['snp_id', 'chrom', 'position', 'REF', 'ALT']])
    # variant_pos[['snp_id', 'chrom', 'position', 'REF', 'ALT']].to_csv('../../data/genotype_information/yeast_snps_loc_dec2022.txt', index=False)



def format_genotypic_matrix(genotype_matrix, barcoded_strains, snps_loc):
    #### Adding strain_ids
    ...
    print('Genotypic matrix:')
    print(genotype_table)
    # genotype_table.to_csv('../../data/genotype_information/piQTL_genotype_matrix_dec2022.txt', index=False)


if __name__ == "__main__":
    ### Required inputs
    genotype_matrix = pd.read_csv('../../data/genotype_information/she_jarosz_2018/phasedGenotype_from_mat.csv', header=None)
    variant_position_matrix = pd.read_csv('../../data/genotype_information/she_jarosz_2018/variantPos_from_vcf.csv', sep='\t', names=['chrom', 'position', 'REF', 'ALT'])
    gene_loc = pd.read_csv('../../data/genome_annotations/sgd_database/orf_coding_R64-3-1.csv')
    ...
    gene_loc = format_gene_loc(gene_loc)

    #### Save snps information table
    snps_loc = format_snps_loc(variant_position_matrix)

    #### Genotype of piQTL subset from She and Jarosz study
    genotype_table = format_genotypic_matrix(genotype_matrix, barcoded_strains, snps_loc)
