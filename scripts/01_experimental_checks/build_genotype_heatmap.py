...
def create_genotype_map(genotype_matrix):
    ...
    if not path.exists(f"../../figures/supp_figure_1"):
        makedirs(f"../../figures/supp_figure_1")

    f.savefig("../../figures/supp_figure_1/FIGURE_S1A.png", dpi=300)
    f.savefig("../../figures/supp_figure_1/FIGURE_S1A.eps", dpi=300)
...
def linkage_disequilibrium_plot(LD_table):
    f, ax = plt.subplots(nrows=1, ncols=1)
    plt.imshow(LD_table.values, vmin=0, vmax=1, extent=[1, 12054, 12054, 1], cmap='binary', interpolation='none', aspect='equal')
    plt.colorbar()

    # f.savefig('../../figures/figure_1/LD_all_chromosomes.png', dpi=200)
    f.savefig('../../figures/figure_1/LD_all_chromosomes.eps', dpi=300)


if __name__ == "__main__":

    genotype_matrix = pd.read_csv(
        f"../../data/genotype_information/piQTL_genotype_matrix_dec2022.txt")
    snp_position = pd.read_csv(
        f"../../data/genotype_information/snps_annotations_genome-version-3-64-1.txt").rename(columns={"snp_id": "SNP"})
    ...
    print("LD distribution across the genome")
    # linkage_disequilibrium_plot(LD_table)