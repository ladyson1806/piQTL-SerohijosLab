...
if __name__ == "__main__":
    all_A_vs_B = pd.read_csv('../../results/02_lineage_tracking/A_vs_B_annotated.csv')
    
    print(len(all_A_vs_B))

    MTX = all_A_vs_B[all_A_vs_B['MTX_condition'].str.contains('_MTX')].reset_index(drop=True)
    noMTX = all_A_vs_B[all_A_vs_B['MTX_condition'].str.contains('_noMTX')].reset_index(drop=True)
    ...
    for TABLE in [all_A_vs_B, MTX, noMTX]:
        ...
        sns.scatterplot(x=REPS['logratio_Fitness_A_REP1'], y=REPS['logratio_Fitness_A_REP2'], hue=REPS['REP_ID'], marker='.', linewidth=0, alpha = 0.7, legend=False, palette=['blue', 'orange', 'green', 'red', 'purple'])
        sns.scatterplot(x=REPS['logratio_Fitness_B_REP1'], y=REPS['logratio_Fitness_B_REP2'], hue=REPS['REP_ID'], marker='.', linewidth=0, alpha = 0.7, legend=False, palette=['blue', 'orange', 'green', 'red', 'purple'])
        ...
        # plt.xlim([-15, 8])
        # plt.ylim([-15, 8])
        plt.annotate(f"Pearson's corr: {round(corr,3)}\npval: {'{:.2e}'.format(pval)}", xy=[0, 270], xycoords='axes points')
        ...
        plt.legend(custom, ['17', '40', '180', '43_REF', '599_REF'], fontsize= 7, loc='upper left')

        f.savefig(f'../../figures/supp_figure_3/duplicate_density_plot_{title[i]}.eps', dpi=300)
        # f.savefig(f'../../figures/supp_figure_3/duplicate_density_plot_{title[i]}.pdf', dpi=300)
        i += 1
