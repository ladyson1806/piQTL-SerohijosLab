...
LI_PQTL_FORMATTED="out/formatted_tables/Li_pQTLs_formatted_Zscore.csv"
if [ ! -e ${LI_PQTL_FORMATTED} ]; then
    echo "Formatting pQTLs from Li et al..."
    python src/format_Li_QTL_table_Zscore.py \
        --input ${LI_PQTL_INPUT} \
...
LI_PQTL_BOXPLOT="out/figures/boxplot_Li_pQTLs_Zscore.png"
if [ ! -e ${LI_PQTL_BOXPLOT} ]; then
    ...
    python src/plot_boxplot.py \
        ...
        --title "Li pQTLs from human study" \
        --y_label "|Z|" \
        --group_by "cis_trans" \
        --y_value "abs_z"
    echo "Done!"
...
## t-Test
LI_TTEST="out/t_test/Li_pQTLs_t-test_Zscore.txt"
if [ ! -e ${LI_TTEST} ]; then
